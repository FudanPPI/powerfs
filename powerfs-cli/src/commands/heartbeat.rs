...
pub async fn heartbeat(mut client: MasterClient, args: HeartbeatArgs) -> super::CommandResult {
    ...
    println!("  IP: {}, Port: {}, GRPC: {}", args.ip, args.port, args.grpc_port);
    println!("  Data Center: {}, Rack: {}", args.data_center, args.rack);
    println!("  Sending {} heartbeats with {}s interval\n", args.count, args.interval);
    ...
    let response_stream = service.send_heartbeat(tonic::Request::new(stream))
        .await
    ...
    let sent_str = if count == 0 { "continuous" } else { &count.to_string() };
    println!("\nCompleted: sent {} heartbeats, received {} responses", sent_str, received);
