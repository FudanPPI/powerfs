...
pub async fn status(mut client: MasterClient, args: StatusArgs) -> super::CommandResult {
    ...
    // Get volume list
    let volume_list = service.volume_list(tonic::Request::new(powerfs_master::proto::VolumeListRequest {}))
        .await
    ...
    if args.detailed {
        ...
        for node in response.data_nodes {
            ...
            for vol in node.volumes {
                println!("      Volume {}: size={}, readonly={}, collection={}", 
                    vol.volume_id, vol.size, vol.read_only, vol.collection);
            }
        }
    } else {
        println!("Total volumes: {}", response.data_nodes.iter().map(|n| n.volumes.len()).sum::<usize>());
    }
