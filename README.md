...
构建前请先运行 `scripts/prep.sh` 生成与当前版本匹配的配置（`powerfs-fuse/Cargo.toml`
...
# 也可手动指定版本
# ./scripts/prep.sh community      # 强制社区版
# ./scripts/prep.sh enterprise     # 强制企业版（需已检出 powerfs-fuse-enterprise）
...
> **注意**：`scripts/prep.sh` 必须在 `cargo build` 之前运行。若直接执行
