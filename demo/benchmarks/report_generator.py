...
<body>
    <div class="container">
        ...
        <div class="summary-grid">
            <div class="summary-card">
                ...
                <div class="summary-value" style="color: #00d9ff;">{len(reports)}</div>
            </div>
            <div class="summary-card">
                <div class="summary-label">Operations</div>
                <div class="summary-value" style="color: #00ff88;">{sum(len(r.get('operations', [])) for r in reports.values())}</div>
            </div>
            <div class="summary-card">
    ...
    for name, report in reports.items():
        ...
        <div class="benchmark-card">
            ...
            <div class="config">
                <span>Config:</span> {json.dumps(report.get('config', {}), indent=2, ensure_ascii=False).replace('\"', "'").replace('\\n', '<br>').replace('    ', '&nbsp;&nbsp;&nbsp;&nbsp;')}
            </div>
            <table>
                <thead>
                    <tr>
                        <th>Operation</th>
                        <th>Rounds</th>
                        { 'AVG OPS/S' if name == 'kv' or name == 'metadata' else 'BW (MB/s)' }
        ...
        for op, data in summary.items():
            html += f"""
                    <tr>
                        <td>{op}</td>
                        <td>{report.get('config', {}).get('rounds', 1)}</td>
                        <td class="{'ops-value' if name == 'kv' or name == 'metadata' else 'bw-value'}">
                            {data.get('avg_ops_per_sec', data.get('avg_bandwidth_mbps', 0)):.2f}
                        </td>
                        <td class="latency-value">{data.get('avg_latency_ms', 0):.4f}</td>
                    </tr>"""
