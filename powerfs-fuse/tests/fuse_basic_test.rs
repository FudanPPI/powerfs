...
fn test_mkdir_readdir_rmdir() {
    ...
    let entries: Vec<_> = fs::read_dir(&test_dir)
        ...
        .collect();
    assert!(entries.contains(&"subdir".to_string()), "subdir should be listed");
    
    let sub_entries: Vec<_> = fs::read_dir(&subdir)
        .expect("Failed to read subdir")
        .map(|e| e.unwrap().file_name().into_string().unwrap())
        .collect();
    assert!(sub_entries.contains(&"file.txt".to_string()), "file.txt should be listed");
...
fn test_write_read_small_file() {
    ...
    let mut file = File::create(&file_path).expect("Failed to create file");
    file.write_all(b"Small test content").expect("Failed to write");
    drop(file);
...
fn test_multilevel_directory_structure() {
    ...
    let mut content = String::new();
    File::open(&file3).unwrap().read_to_string(&mut content).unwrap();
...
fn test_append_write() {
    ...
    let mut file = File::create(&file_path).expect("Failed to create file");
    file.write_all(b"First part").expect("Failed to write first part");
    drop(file);
...
fn test_truncate_file() {
    ...
    let mut file = File::create(&file_path).expect("Failed to create file");
    file.write_all(b"This is a longer text").expect("Failed to write");
    drop(file);
