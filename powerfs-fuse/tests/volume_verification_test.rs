...
fn test_write_immediate_read() {
    ...
    assert_eq!(read_content, content, "Content should be readable immediately after write");
}
...
fn test_write_flush_read() {
    ...
    let read_content = fs::read_to_string(&test_file).expect("Failed to read");
    assert_eq!(read_content, content, "Content should be readable after flush");
}
